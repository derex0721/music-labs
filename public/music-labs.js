...
const discoverItems=[
  ...
  {category:'Music Tech',code:'MIDI',title:'Roland Melody Flip：用 AI 生成可編輯 MIDI',summary:'依曲風、BPM 與調性產生旋律、和弦、Bass 和鼓組，再拖回 DAW 繼續製作。',date:'2026-09-05',source:'The Verge',url:'https://www.theverge.com/ai-artificial-intelligence/990197/roland-ai-music-melody-flip'},
...
async function submitNetlifyForm(form){const button=form.querySelector('button[type="submit"]');const status=form.querySelector('[data-form-status]');const success=document.getElementById('feedbackSuccess');button.disabled=true;button.dataset.label=button.textContent;button.textContent='SENDING…';status.textContent='正在提交…';try{const response=await fetch('/__forms.html',{method:'POST',headers:{'Content-Type':'application/x-www-form-urlencoded'},body:new URLSearchParams(new FormData(form)).toString()});if(!response.ok)throw new Error();form.reset();form.hidden=true;success.hidden=false}catch{status.textContent='暫時無法送出，請稍後再試或使用頁尾 Email。';button.disabled=false;button.textContent=button.dataset.label}}
document.getElementById('feedbackForm')?.addEventListener('submit',event=>{event.preventDefault();if(event.currentTarget.reportValidity())submitNetlifyForm(event.currentTarget)});
